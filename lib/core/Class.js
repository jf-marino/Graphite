	...
	namespace.Class.extend = function(definition) {
		...
		if(isValid("object", definition)) {
			...
			if(hasProp("constructor", definition)) {
                var constructorName = definition.constructor.name ? definition.constructor.name+"Proxy" : "ConstructorProxy";
				var defConstructor = definition.constructor;
				eval("_constructor = function " + constructorName + "() { " +
					"this._super = __super.constructor; " +
					"var retValue = defConstructor.apply(this, arguments); " +
                    "this._super = __super; " +
					"return retValue; " +
				"};");
			}
				...
				if(name !== "constructor" && name !== "static") {
					...
					if(isValid("function", value)) {
						...
						if(isValid("function", __super[name])) {
                            ...
                            var fnName = value.name ? value.name+"Proxy" : "MethodProxy";
							var fn = value;
							eval("_constructor.prototype[name] = function " + fnName + "() { " +
